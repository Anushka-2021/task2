...
class RussianNames:
    ...
    def _load_set(self, section, max_len, first_letter='0'):
        if first_letter == '0':
            return list(filter(lambda x: len(x) <= max_len, section.split(' ')))
        else:
          #  print(lambda x: len(x) <= max_len and x[0][0] == first_letter and print(x[0]), section.split(' '))
            return list(filter(lambda x: len(x) <= max_len and x[0] == first_letter, section.split(' ')))
    ...
    def _fill_base(self):
        ...
        self._base = {
            'man': {
                'surname': self._load_set(surnames_m, self.surname_max_len, self.first_letter),
    ...
    def _get_object(self, gender, elem_type, reduction=False, first_letter='0'):
    ...
    def get_person(self, **kwargs):
        ...
        surname = self._get_object(gender, 'surname', self.surname_reduction, self.first_letter)
        person = {
            ...
            'gender': 'male' if gender == True else 'female',
        }
